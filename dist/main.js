...
const enemies_js_1 = require("./gameModules/enemies.js");
const towers_js_1 = __importDefault(require("./gameModules/towers.js"));
const gameConfig_js_1 = require("./gameModules/gameConfig.js");
const waves_js_1 = require("./gameModules/waves.js");
const controls_js_1 = require("./gameModules/controls.js");
// ------------------- Canvas Setup -------------------
...
const gold = { value: gameConfig_js_1.INITIAL_GOLD };
let score = gameConfig_js_1.INITIAL_SCORE;
...
function initializeGame() {
    selectedMapIndex = Math.floor(Math.random() * gameConfig_js_1.MAPS.length);
    rawPath = gameConfig_js_1.MAPS[selectedMapIndex];
    path = scalePath(rawPath);
    enemies = [];
    towers = [];
    projectiles = [];
    lives = gameConfig_js_1.INITIAL_LIVES;
    score = gameConfig_js_1.INITIAL_SCORE;
    gold.value = gameConfig_js_1.INITIAL_GOLD; // Reset gold without reassigning
    gameOverFlag = false;
    gameWonFlag = false;
    currentWaveIndex = 0;
...
function generateRandomSpawnOffset(path, enemySize) {
    ...
    const halfPathWidth = gameConfig_js_1.PATH_WIDTH / 2;
...
function startNextWave() {
    if (currentWaveIndex >= waves_js_1.waves.length) {
        // All waves completed
        gameWonFlag = true;
    ...
    const wave = waves_js_1.waves[currentWaveIndex];
    ...
    currentWaveSpawnGroups.slice().forEach((spawnGroup, index) => {
        if (!spawnGroup.active) {
            ...
            if (spawnGroup.spawnTimer <= 0) {
                ...
                console.log(`SpawnGroup started: ${spawnGroup.class.name}, Count: ${spawnGroup.remainingCount}`);
            }
        }
        else {
            ...
            if (spawnGroup.spawnTimer <= 0 && spawnGroup.remainingCount > 0) {
                // Spawn an enemy
                let enemySize;
                if (spawnGroup.class === enemies_js_1.BasicEnemy) {
                    enemySize = 20;
                }
                else if (spawnGroup.class === enemies_js_1.FastEnemy) {
                    enemySize = 10;
                }
                else if (spawnGroup.class === enemies_js_1.TankEnemy) {
                    enemySize = 30;
                }
                else {
                ...
                console.log(`Spawned ${spawnGroup.class.name}. Remaining: ${spawnGroup.remainingCount}`);
...
function drawPaths() {
    ...
    context.lineWidth = gameConfig_js_1.PATH_WIDTH;
    context.lineCap = 'round';
    gameConfig_js_1.MAPS.forEach((singlePath, index) => {
...
function render() {
    ...
    context.fillText(`Wave: ${currentWaveIndex < waves_js_1.waves.length ? waves_js_1.waves[currentWaveIndex].number : 'N/A'}`, 10, 120);
    context.fillText(`Enemies Remaining: ${enemies.length}`, 10, 150);
    // Render the tower preview as a square
    const preview = (0, controls_js_1.getPreview)();
    if (preview.visible) {
        context.fillStyle = preview.color;
        context.fillRect(preview.x - gameConfig_js_1.TOWER_SIZE / 2, preview.y - gameConfig_js_1.TOWER_SIZE / 2, gameConfig_js_1.TOWER_SIZE, gameConfig_js_1.TOWER_SIZE);
    }
}
/**
...
function displayGameOver() {
    ...
    context.fillText('Tap/Click to Restart', canvas.width / 2, canvas.height / 2 + 60);
...
function addTower(x, y) {
    towers.push(new towers_js_1.default(x, y, context, enemies));
}
/**
...
(0, controls_js_1.initControls)(canvas, {
    gold,
    deductGold,
    addTower,
    isOnPath,
...
function isOnPath(x, y) {
    const halfPathWidth = gameConfig_js_1.PATH_WIDTH / 2;
    const towerHalfSize = gameConfig_js_1.TOWER_SIZE / 2;
    for (let singlePath of gameConfig_js_1.MAPS) {
        const scaledPath = scalePath(singlePath);
...
function isOnTower(x, y) {
    for (let tower of towers) {
        ...
        const dy = y - tower.y;
        const distance = Math.hypot(dx, dy);
        if (distance < gameConfig_js_1.TOWER_SIZE) {
