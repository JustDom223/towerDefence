 ...
 */
function initControls(canvas, dependencies) {
    const { gold, deductGold, addTower, 
    // displayNotEnoughGold,
    // displayCannotPlaceHere,
    isOnPath, isOnTower, isGameActive, // New dependency
     } = dependencies;
    ...
    canvas.addEventListener('mousemove', function (event) {
        if (isTouchActive)
            return; // Ignore mouse events if touch is active
        if (!isGameActive()) {
    ...
    canvas.addEventListener('click', function (event) {
        if (isTouchActive)
            return; // Ignore mouse clicks if touch is active
        if (!isGameActive())
            return; // Ignore tower placement if game is not active
        const rect = canvas.getBoundingClientRect();
        const scaleX = canvas.width / rect.width;
        const scaleY = canvas.height / rect.height;
        const x = (event.clientX - rect.left) * scaleX;
