...
const enemies_js_1 = require("./enemies.js");
...
function generateWaves(totalWaves) {
    ...
    for (let i = 1; i <= totalWaves; i++) {
        ...
        wave.enemies.push({ class: enemies_js_1.BasicEnemy, count: basicEnemyCount });
        // Introduce FastEnemy starting from wave 5
        if (i >= 5) {
            const fastEnemyCount = 5 + Math.floor(i / 4);
            wave.enemies.push({ class: enemies_js_1.FastEnemy, count: fastEnemyCount });
        }
        // Introduce TankEnemy starting from wave 10
        if (i >= 10) {
            const tankEnemyCount = 3 + Math.floor(i / 10);
            wave.enemies.push({ class: enemies_js_1.TankEnemy, count: tankEnemyCount });
        }
...
exports.waves = generateWaves(30);
